...
// ─────────────────────────────────────────────────────────────────────────────
// AFFICHER LES CARTES (sans boutons Modifier / Supprimer)
// ─────────────────────────────────────────────────────────────────────────────
function afficherSalles(filtreSalle = "", filtreType = "") {
    document.getElementById("roomsContainer").innerHTML = "";

    const sallesFiltrees = salles.filter(ele => {
        const okSalle = filtreSalle === "" || "Salle " + ele.nom_salle === filtreSalle;
        const okType  = filtreType  === "" || ele.type_salle === filtreType;
        ...
        document.getElementById("roomsContainer").innerHTML += `
            <div class="col-md-6 col-lg-4" data-aos="fade-up">
                <div class="room-card">
                    ...
                    <!-- ✅ Seul le bouton Matériel reste, Modifier et Supprimer supprimés -->
...
// ─────────────────────────────────────────────────────────────────────────────
// OUVRIR MODAL MATÉRIEL (consultation seule)
// ─────────────────────────────────────────────────────────────────────────────
async function ouvrirMateriel(id, nomSalle) {
    ...
    try {
        ...
        if (!mat.length) {
            document.getElementById('modalMatBody').innerHTML = `
                <div class="text-center py-4" style="color:#9ca3af">
                    <i class="fas fa-box-open" style="font-size:2.5rem;opacity:.35;display:block;margin-bottom:.8rem"></i>
        ...
        document.getElementById('modalMatBody').innerHTML = `
            <div class="mat-stats">
                <span class="mat-stat-chip chip-blue"><i class="fas fa-boxes"></i> ${total} élément${total > 1 ? 's' : ''}</span>
                <span class="mat-stat-chip chip-green"><i class="fas fa-check-circle"></i> ${dispo} disponible${dispo > 1 ? 's' : ''}</span>
                ${occupe ? `<span class="mat-stat-chip chip-red"><i class="fas fa-times-circle"></i> ${occupe} occupé${occupe > 1 ? 's' : ''}</span>` : ''}
                ${maint  ? `<span class="mat-stat-chip chip-warn"><i class="fas fa-tools"></i> ${maint} en maintenance</span>` : ''}
            </div>
            <div style="overflow-x:auto">
                <table class="mat-table">
                    <thead>
                        <tr><th>#</th><th>Nom</th><th>Type</th><th>État</th><th>Quantité</th><th>Remarques</th></tr>
                    </thead>
                    <tbody>
                        ${mat.map(m => `
                        <tr>
        ...
        document.getElementById('modalMatBody').innerHTML = `
            <div class="text-center py-4" style="color:#ef4444">
                <i class="fas fa-exclamation-triangle" style="font-size:2rem;display:block;margin-bottom:.8rem"></i>
                Erreur : ${e.message}
            </div>`;
    }
}
